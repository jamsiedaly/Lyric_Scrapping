...
import os
from lyric_blobs import lyric_blobs
...
def scrap_artist(artist_name, artist_URL):
    ...
    scraper = lyric_blobs()
    song_file = scraper.create_files("plain_text", " of ", artist_name)
...
for i in range(0, 100):
    for i, artist in enumerate(artist_names):
        ...
        while not scraped:
            try:
                scrap_artist(artist, artist_URLs[i])
                scraped = True
            except:
                scraped = False
